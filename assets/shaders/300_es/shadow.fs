...
float SampleShadow(int cascade) {
    ...
    for (int x = -kernelSize; x <= kernelSize; x++) {
        for (int y = -kernelSize; y <= kernelSize; y++) {
            ...
            if (cascade == 0) {
                sampleDepth = texture(shadowMap[0], sampleCoords + offset).r;
            } else if (cascade == 1) {
                 sampleDepth = texture(shadowMap[1], sampleCoords + offset).r;
             } else if (cascade == 2) {
                 sampleDepth = texture(shadowMap[2], sampleCoords + offset).r;
             } else if (cascade == 3) {
                 sampleDepth = texture(shadowMap[3], sampleCoords + offset).r;
             }
